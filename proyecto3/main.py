...
app = FastAPI()
articulos: Dict[int, dict] = {} # diccionario vacio

class ArticuloBase(BaseModel):
    nombre: str
    descripcion: str | None = None  # La descripcion es opcional
    precio: float
...
def obtener_todos_articulos():
    if not articulos:
        return JSONResponse(
            status_code=404,
            content={"exitos": False, "mensaje": "No se encotraron articulos"}
        )
    lista_articulos = [ArticuloRespuesta(id=articulo_id, **datos).model_dump() for articulo_id, datos in articulos.items()]
    return JSONResponse(
        status_code=200,
        content={"exitos": True, "articulos": lista_articulos}
    )

@app.get("/articulos/{id}", response_model=ArticuloRespuesta)
def obtener_articulo(articulo_id: int):
    ...
    return JSONResponse(
        status_code=200,
        content={"exitos": True, "articulo": ArticuloRespuesta(id=articulo_id, **articulos[articulo_id]).model_dump()}
    )
...
def eliminar_articulo(articulo_id: int):
    ...
    return JSONResponse(
        ...
        content={
            "exitos": True,
            "mensaje": "Articulo {articulo_id} eliminado",
